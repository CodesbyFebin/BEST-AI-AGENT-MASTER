...
export async function generateMetadata({ params }: P): Promise<Metadata> {
  ...
  const title = `${name} — evidence & sources`;
  return {
    title,
    description: summary,
    alternates: { canonical: `/models/${slug}` },
    robots: { index: Boolean(entity?.verification === "verified"), follow: true },
    openGraph: { title, description: summary, url: `/models/${slug}`, type: "article" },
...
export default async function Page({ params }: P) {
  ...
  if (!entity) {
    ...
    return <div className="shell detail">
      ...
      <JsonLd data={{ "@type": "WebPage", name: legacy.name, url: `${SITE.url}/models/${legacy.slug}`, about: { "@type": "SoftwareApplication", name: legacy.name, url: legacy.sourceUrl }, breadcrumb: { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: SITE.url }, { "@type": "ListItem", position: 2, name: "Models", item: `${SITE.url}/models` }, { "@type": "ListItem", position: 3, name: legacy.name, item: `${SITE.url}/models/${legacy.slug}` }] } }} />
